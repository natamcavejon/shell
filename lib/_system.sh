...
system_create_user() {
  ...
  mv "${PROJECT_ROOT}"/wpp_zdg.zip /var/www/html/
EOF
...
system_clone_wpp() {
  print_banner
  printf "${WHITE} 💻 Fazendo clone do wppconnect-server...${GRAY_LIGHT}"
  printf "\n\n"
...
#   None
#######################################
system_unzip_wpp_zdg() {
  print_banner
  printf "${WHITE} 💻 Fazendo unzip wpp_zdg...${GRAY_LIGHT}"
  printf "\n\n"
...
cat > /etc/nginx/conf.d/wpp.conf << 'END'
