...
<body>
  ...
  <?php
	...
	function button9() {
		$tokenGrupos= $_POST['tokenGrupos'];
		$sessaoGrupos = $_POST['sessaoGrupos'];
		$authorizationGrupos = "Authorization: Bearer " . $tokenGrupos;
		$urlGrupos= "http://localhost:21465/api/" . $sessaoGrupos . "/all-groups";	
		$curl = curl_init();
		curl_setopt_array($curl, array(
		  CURLOPT_URL => $urlGrupos ,
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => '',
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
		...
		$array = array_column($responseJ["response"], "id");
		foreach (array_column($array, "_serialized") as $arr){
			echo $arr . '<br>';
		}
		echo "<hr>";
	}
	...
	echo "<button class='nav-link' id='nav-9-tab' data-toggle='tab' data-target='#nav-9' type='button' role='tab' aria-controls='nav-9' aria-selected='false'>Listar Grupos</button>";
    echo "</div>";
	echo "</nav>";
	...
	echo "<hr>";
	echo "</div>";
	echo "<div class='tab-pane fade' id='nav-9' role='tabpanel' aria-labelledby='nav-9-tab'>";
	echo "<br>";
	echo "<h4>Listar Grupos</h4>";
	echo "<input style='width:20%' class='form-control'type='text' name='sessaoGrupos' id='sessaoGrupos' placeholder='Insira a Sessão'>";
	echo "<br>";
	echo "<input style='width:20%' class='form-control'type='text' name='tokenGrupos' id='tokenGrupos' placeholder='Insira o Token'>";
	echo "<br>";
	echo "<input class='btn btn-success' type='submit' name='button9' value='Listar' />";
	echo "<br>";
	echo "<hr>";
	echo "</div>";
	echo "</div>";
